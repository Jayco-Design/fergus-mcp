...
export const manageQuotesToolDefinition = {
  name: 'manage-quotes',
  description: 'Manage quotes. Actions: get, get-by-guid, get-detail, list (per job), list-all (across jobs), create, update, update-version, get-totals, publish, mark-as-sent, accept, decline, void. The wrapper preserves title/description on updates. Only draft quotes can be updated.',
  inputSchema: {
    type: 'object',
    properties: {
      action: {
        type: 'string',
        enum: ['get', 'get-by-guid', 'get-detail', 'list', 'list-all', 'create', 'update', 'update-version', 'get-totals', 'publish', 'mark-as-sent', 'accept', 'decline', 'void'],
      ...
      publishedAt: {
        type: 'string',
        description: 'Publish timestamp in ISO 8601 format. Defaults to now. (for: publish)',
      },
      // decline params
      declinedAt: {
        type: 'string',
        description: 'Decline timestamp in ISO 8601 format. Defaults to now. (for: decline)',
      },
      reasonNotes: {
        type: 'string',
      ...
      rejectedBy: {
        type: 'string',
        description: 'Name of person who declined. (for: decline)',
      },
      // void params
      voidedAt: {
        type: 'string',
        description: 'Void timestamp in ISO 8601 format. Defaults to now. (for: void)',
      },
    },
    required: ['action'],
  },
...
export async function handleManageQuotes(
  ...
  switch (args.action) {
    ...
    default:
      throw new Error(`Unknown action: ${args.action}. Valid actions: get, get-by-guid, get-detail, list, list-all, create, update, update-version, get-totals, publish, mark-as-sent, accept, decline, void`);
  }
}
...
async function handlePublishQuote(
  ...
  const { quoteId, publishedAt } = args;
  if (!quoteId) {
...
async function handleMarkQuoteAsSent(
  ...
  const result = await fergusClient.post(`/jobs/quotes/${quoteId}/markAsSent`, {});
  return {
    content: [{ type: 'text' as const, text: JSON.stringify(result, null, 2) }],
  };
}
...
async function handleDeclineQuote(
  ...
  const { quoteId, declinedAt, reasonNotes, rejectedBy } = args;
  if (!quoteId) {
    throw new Error('quoteId is required for decline action');
  }

  const requestBody: Record<string, unknown> = {};
  if (declinedAt) requestBody.declinedAt = declinedAt;
  if (reasonNotes) requestBody.reasonNotes = reasonNotes;
  if (rejectedBy) requestBody.rejectedBy = rejectedBy;
...
async function handleVoidQuote(
  ...
  const { quoteId, voidedAt } = args;
  if (!quoteId) {
    throw new Error('quoteId is required for void action');
  }

  const requestBody: Record<string, unknown> = {};
  if (voidedAt) requestBody.voidedAt = voidedAt;
