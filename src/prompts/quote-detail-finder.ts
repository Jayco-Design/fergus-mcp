...
2. Fetch recent jobs with \`manage-jobs\` action \`list\` (limit ~25, newest first) and look for matches in the job title or related customer/site fields that mention "${searchTerm}".
3. For matching jobs, call \`manage-quotes\` with action \`list\` and the \`jobId\` to find associated quotes. If no job match, try \`manage-quotes\` with action \`list-standalone\` (pageSize ~25) and filter client-side.
